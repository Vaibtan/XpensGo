...
The replacement runtime under `apps/` and `packages/` has a provider-backed platform tracer plus identity and Telegram adapter foundations. The Next.js workspace resolves a Better Auth session into an application-owned User, personal Ledger, timezone, and visible Telegram link state through the Effect API. The staging Telegram webhook is registered and has deployed invalid-secret rejection, duplicate ingress, asynchronous processing, and explicit Bot API rejection evidence. Durable operator recovery is implemented for explicit terminal failures only; ambiguous or accepted calls cannot be requeued. See the [platform staging report](./docs/evidence/phase1-staging.md), [identity foundation evidence](./docs/evidence/identity-foundation.md), [local Telegram adapter evidence](./docs/evidence/telegram-adapter-local.md), and [Telegram staging evidence](./docs/evidence/telegram-staging.md). Real Bot API acceptance from a valid private chat, live operator recovery execution, recovery email, transactions, deployed identity acceptance, and production readiness remain open; this is not yet a user-ready product.
