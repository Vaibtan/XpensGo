...
| Hono + `@hono/zod-openapi`   | Excellent Cloudflare support, mature middleware/routing, OpenAPI and RPC patterns.                                                           | Adds Hono plus Zod as a second schema/error system, or relies on a third-party bridge for Effect Schema; handlers still need an explicit Effect execution seam. | Rejected; no parallel router or schema system is installed or maintained. [Hono Cloudflare guide](https://hono.dev/docs/getting-started/cloudflare-workers), [Hono OpenAPI example](https://hono.dev/examples/zod-openapi), [Hono RPC](https://hono.dev/docs/guides/rpc) |
...
If either selected stack fails its Worker compatibility, lifecycle, cookie, or contract-generation gate, stop the implementation ticket and reopen the architectural decision. Do not add an alternate authentication provider, router, or schema framework as a standby dependency.
