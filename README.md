...
The replacement runtime under `apps/` and `packages/` has a provider-backed platform tracer plus locally verified identity and Telegram adapter foundations. The Next.js workspace resolves a Better Auth session into an application-owned User, personal Ledger, timezone, and visible Telegram link state through the Effect API. The API Worker verifies and normalizes private Telegram text updates, resolves Channel Identity server-side, applies persisted abuse controls, and creates durable semantic replies with duplicate-safe provider-attempt state. See the [platform staging report](./docs/evidence/phase1-staging.md), [identity foundation evidence](./docs/evidence/identity-foundation.md), and [Telegram adapter evidence](./docs/evidence/telegram-adapter-local.md). Staging webhook registration, real Bot API delivery evidence, recovery email, transactions, deployed identity acceptance, and production readiness remain open; this is not yet a user-ready product.
