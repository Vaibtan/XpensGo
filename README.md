...
The replacement runtime under `apps/` and `packages/` has a provider-backed platform tracer plus a locally verified identity foundation. The Next.js workspace now resolves a Better Auth session into an application-owned User, personal Ledger, timezone, and visible Telegram link state through the Effect API; hashed one-use link and unlink challenges preserve ownership across safe relinking. See the [platform staging report](./docs/evidence/phase1-staging.md) and [identity foundation evidence](./docs/evidence/identity-foundation.md). Telegram ingress and replies, recovery email, transactions, deployed identity acceptance, and production readiness remain open; this is not yet a user-ready product.
