...
- The local outbox application operation claims a bounded batch, sends content-minimized identifiers outside the database transaction, marks publication only after Queue acceptance, and durably defers typed failures. Publication has a five-second timeout, five total attempts, capped exponential backoff, a terminal database state, and an explicit `db:outbox:retry` operator command. PostgreSQL claim leases use `FOR UPDATE SKIP LOCKED`; a ten-minute reconciler re-enqueues or terminalizes published messages without receipts, while a unique receipt suppresses duplicate processing and reconciles send-succeeded/mark-failed ambiguity. Queue batches decode before dependency selection: status jobs never acquire PostgreSQL, while one scoped four-connection Layer serves the outbox subset and persistence failure retries only those messages before the configured Cloudflare dead-letter path.
...
- The dependency gate accepts exactly 5 reviewed package findings (3 high, 2 moderate) covered by 4 current upstream advisories in the pinned Next.js/OpenNext dependency tree. The allowlist expires on 2026-08-15 and fails closed on new findings, expiry, severity or directness changes, advisory metadata changes, and installed-version drift; forced downgrades and unverified transitive overrides remain prohibited.
