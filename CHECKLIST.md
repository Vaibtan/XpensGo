...
**Status:** Phase 1 minimal platform tracer is in progress; the TypeScript, Effect, PostgreSQL, recoverable outbox, Neon/Hyperdrive, Queue, and deployed Worker foundations are implemented, while authentication, the CI database, and deployed Hyperdrive-to-Queue proofs remain open.
...
- `npm run check` passes formatting, type-aware lint, strict type-checking, 20 behavioral tests, package declaration builds, an API Worker dry-run bundle, a Next.js production build, and an OpenNext Cloudflare bundle.
...
- `npm run test:integration` passes 14 tests against local PostgreSQL 17.10 and the Worker runtime: deterministic empty-database migration, repeatability, column-level runtime-role separation, concurrent inbound and publication idempotency, cross-owner rejection, transaction rollback, bounded failed-publication recovery, ambiguous-send receipt reconciliation, stalled-consumer reconciliation, duplicate-safe consumption, per-message retry isolation, and the real `scheduled` and `queue` entrypoints. A deterministic adapter test also proves the five-second Queue timeout branch.
...
- The local outbox application operation claims a bounded batch, sends content-minimized identifiers outside the database transaction, marks publication only after Queue acceptance, and durably defers typed failures. Publication has a five-second timeout, five total attempts, capped exponential backoff, a terminal database state, and an explicit `db:outbox:retry` operator command. PostgreSQL claim leases use `FOR UPDATE SKIP LOCKED`; a ten-minute reconciler re-enqueues or terminalizes published messages without receipts, while a unique receipt suppresses duplicate processing and reconciles send-succeeded/mark-failed ambiguity. Per-message processing failures use bounded Queue retry; invocation Layer failure retries the batch before the configured Cloudflare dead-letter path.
- A PostgreSQL-backed local integration drives an accepted inbound transaction through the real Worker `scheduled` entrypoint, a recording Queue binding, and duplicate deliveries through the real Worker `queue` entrypoint. It proves local application composition and recovery without claiming an external Cloudflare Queue or deployed Hyperdrive result.
- `npm audit --audit-level=high` is enabled but currently reports 5 upstream findings (3 high, 2 moderate) in the pinned Next.js/OpenNext dependency tree. This keeps the CI/security portion of the tooling item open; forced downgrades or unverified transitive overrides are not accepted as remediation.
...
- No managed execution of outbox migrations 0002–0003, deployed Hyperdrive query or transaction, external Queue publication/recovery, authenticated route, Telegram behavior, or production-recovery claim is made by this evidence.
